...
max_iteration = 10                                             # Anzahl der Durchlaeufe
...
sequence_length =  [100,200,300,400,500,600,700]
inputsize = sequence_length[rank]                                     # inputsize = input_size[rank//3]  die ersten drei Ranks, dann die naechsten 3 usw


T_train = int(0.1*T_plastic)                                         # Anzahl der Timesteps ohne Lernen (eta_STDP = 0) #int(0.1*T_plastic)
...
for k in range(max_iteration):
    ...
    W_eu = copy(W_eu_pre[:,round(rand()*Input-1)][:,newaxis])
    ...
    for t in range(Timesteps):
        ...
        #connections[t] = size(W_ee[W_ee!=0])
